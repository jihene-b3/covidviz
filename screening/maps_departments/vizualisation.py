...
grand_public_path = 'screening/data/sites-prelevements-grand-public.csv'
acces_restreint_path = 'screening/data/sites-prelevements-acces-restreint.csv'
...
dep_fr.drop(['Unnamed: 4', 'Unnamed: 5', 'Unnamed: 6', 'Unnamed: 7'],
            axis=1, inplace=True)
...
for dep_code in dep_fr['maille_code'].tolist():
    for i in range(len(depis_department_grand_public[f'{dep_code}'])):
        ...
        folium.Marker(
            ...
            icon=icon,
            popup=popup,
            tooltip=tooltip).add_to(all_map_dep[f'{dep_code}'])
