...
def test_format_granularity():
    df_covid_dep = cm.choose_granularity(df_covid,"departement")
    df_covid_reg = cm.choose_granularity(df_covid,"region")
    test_reg = cm.format_granularity(df_covid_reg, "region")
    test_dep = cm.format_granularity(df_covid_dep, "departement")
    print(test_dep["maille_code"])
    assert (test_reg["maille_code"][20] == "11")
    assert (test_dep["maille_code"][5] == "16")

def test_choose_columns():
    test = cm.choose_columns(df_covid, ["date", "maille_code"])
    assert list(test.columns) == ["date", "maille_code"]


def test_choose_granularity():
    test1 = cm.choose_granularity(df_covid, "departement")
    test2 = cm.choose_granularity(df_covid, "region")
    assert list(test1.granularite.unique())[0] == "departement"
    assert list(test2.granularite.unique())[0] == "region"

def test_Map_covid():
    test = cm.Map_covid(df_covid, departments, "deces")
