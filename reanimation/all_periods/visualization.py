...
rld.rea_all.drop(['deces', 'hospitalises', 'nouvelles_hospitalisations', 'nouvelles_reanimations', 'gueris'],1, inplace = True)

rld.rea_all = rld.rea_all.set_index('date')
rld.rea_all = rld.rea_all.sort_index()
rld.rea_all = rld.rea_all.loc['2020-03-18':]
rld.rea_all = rld.rea_all.loc[rld.rea_all['source_nom']=="Santé publique France Data",:]
...
def rea_DEP_nom(department):
    x = pd.DataFrame(rld.rea_all.loc[rld.rea_all['maille_nom']==f"{department}",'reanimation']).resample("1D").sum()
    x = x.rename(columns = {"reanimation": f"{department}"})
    return(x)


def rea_DEP_code(num_dep):
    x = pd.DataFrame(rld.rea_all.loc[rld.rea_all['maille_code']==f"DEP-{num_dep}",'reanimation']).resample("1D").sum()
...
rea_all_regions["Provence-Alpes-Côte-d'Azur"] = rea_all_Provence_Alpes_Côte_Azur
...
rld.beds_in_rea_reg = rld.beds_in_rea_reg.rename(columns={'Unnamed: 0' : 'Région'})
rld.beds_in_rea_reg = rld.beds_in_rea_reg.drop(['CHR', 'Autres'], 1)

rld.beds_in_rea_reg = rld.beds_in_rea_reg.loc[:13]
rld.beds_in_rea_reg = rld.beds_in_rea_reg.rename(columns={'Confondu' : 'Nombre de lits'})
rld.beds_in_rea_reg = rld.beds_in_rea_reg.reindex(index=[2,8,11,3,1,5,6,7,9,0,10,12,4,13])
...
for col in Regions :
    for row in range(len(rea_reg_prop)) :
        rea_reg_prop.loc[row, f'{col}'] = rea_reg_prop.loc[row, f'{col}']/int(rld.beds_in_rea_reg.loc[f'{col}','Nombre de lits'])
...
for col in rld.beds_in_rea_dep.index :
    for row in range(len(rea_dep_prop)) :
        rea_dep_prop.loc[row, f'{col}'] = rea_dep_prop.loc[row, f'{col}']/int(rld.beds_in_rea_dep.loc[f'{col}','Lits rea dispo'])

df_reg = rea_reg_prop.iloc[:,1:].copy()
df_dep = rea_dep_prop.iloc[:,1:].copy()

def rea_sat_reg(df_reg):
    fig = px.imshow(df_reg, y=rea_reg_prop['date'], title='Regional saturation in reanimation service', height=700, width=800)
    return(fig.show())

def rea_sat_dep(df_dep):
    fig = px.imshow(df_dep, y=rea_reg_prop['date'], title='Regional saturation in reanimation service', height=700, width=800)
    return(fig.show())
